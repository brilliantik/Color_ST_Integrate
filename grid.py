...
from ImageCreate import get_params
import math
...
def gu_reggrid(x0, y0, x1, y1, nx, ny):
	...
	elem_vert = [
		[j + i * nx, (j + 1) + i * nx, (j + 1) + (i + 1) * nx, j + (i + 1) * nx]
...
def gu_reggrid_tr(x0, y0, x1, y1, nx, ny):
	...
	# elem_vert1 = list()
	elem_vert1 = [
		[[j + i * nx, (j + 1) + i * nx, j + ny * nx + i * (nx - 1)],
...
def assigment_color_to_vert(grid, img, config):
	...
	vc = []
	for i in range(grid.Nvert):
		x = (grid.vert[i][0] - x_0) * (pixel_max - 1) / norm_xy
		y = (grid.vert[i][1] - y_0) * (pixel_max - 1) / norm_xy
		# x = int((grid.vert[i][0] - x_0) * (pixel_max - 1) / norm_xy)
		# y = int((grid.vert[i][1] - y_0) * (pixel_max - 1) / norm_xy)
		# print('x = ', x, ' y = ', y, ' i = ', i)
